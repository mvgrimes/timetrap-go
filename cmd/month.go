...
in this`)
	...
	viper.BindPFlag("ids", monthCmd.PersistentFlags().Lookup("ids"))
	viper.BindPFlag("start", monthCmd.PersistentFlags().Lookup("start"))
	viper.BindPFlag("format", monthCmd.PersistentFlags().Lookup("format"))
}
